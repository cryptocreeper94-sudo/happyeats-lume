...
import {
  ArrowLeft, Download, Copy, Check, Palette, Type, Image,
  Shield, Sparkles, ExternalLink, Layers, FileImage, Star, Share2, Mail
} from "lucide-react";
...
const BRAND_LOGOS = [
  {
    id: "smiley",
    name: "Happy Eats Smiley",
    desc: "Primary logo mark — best for business cards, social profiles, and app icons",
    src: "/happyeats-smiley-hires.png",
    category: "Primary",
    formats: ["PNG"],
    bestFor: "Cards, Social, Avatars",
    gradient: "from-orange-500/20 to-rose-500/20",
    border: "border-orange-500/30",
  },
  {
    id: "icon-full",
    name: "Happy Eats Retro Icon",
    desc: "Full retro-styled poster logo with brand wordmark — ideal for print materials and signage",
    src: "/happyeats-icon.png",
    category: "Full Logo",
    formats: ["PNG"],
    bestFor: "Print, Signage, Merch",
  ...
  },
  {
    id: "app-icon",
    name: "App Badge Icon",
    desc: "Square app icon with gradient background — use for app store listings and favicons",
    src: "/icon-512.png",
    category: "App Icon",
    formats: ["PNG"],
    bestFor: "App Store, Favicon",
    gradient: "from-violet-500/20 to-fuchsia-500/20",
    border: "border-violet-500/30",
  },
  {
    id: "tl-badge",
    name: "Trust Layer Emblem",
    desc: "Trust Layer ecosystem verification badge — for compliance documents and partnership materials",
    src: "/trustlayer-emblem.jpg",
    category: "Ecosystem",
    formats: ["JPG"],
    bestFor: "Documents, Partnerships",
    gradient: "from-cyan-500/20 to-blue-500/20",
    border: "border-cyan-500/30",
  },
  {
    id: "tl-icon",
    name: "TL Driver Connect Icon",
    desc: "Trust Layer Driver Connect app icon — for franchise sales demos and white-label materials",
    src: "/tl-icon-512.png",
    category: "Ecosystem",
    formats: ["PNG"],
    bestFor: "Franchise Sales, Demo",
...
const TYPOGRAPHY = [
  { name: "System Sans", family: "system-ui, -apple-system, sans-serif", weight: "400–900", usage: "Primary UI text, all body content", sample: "Happy Eats Nashville" },
...
const GUIDELINES = [
  { title: "Clear Space", desc: "Always maintain padding equal to the logo height around the logo mark. Never crowd it against edges.", icon: "📐" },
  { title: "Minimum Size", desc: "Smiley logo: never smaller than 32px on screen or 0.5\" in print.", icon: "📏" },
  { title: "Dark Backgrounds", desc: "Use smiley or retro icon. Orange pops best against navy (#0f172a) or black.", icon: "🌙" },
  { title: "Light Backgrounds", desc: "Use full retro icon or app badge. Add subtle drop shadow if needed.", icon: "☀️" },
  { title: "No Modifications", desc: "Don't rotate, skew, recolor, or add effects. Use logos exactly as provided.", icon: "🚫" },
  { title: "Print Ready", desc: "All logos are 512px+ and suitable for cards, flyers, wraps, and merch.", icon: "🖨️" },
];
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/owner">
          <Button variant="ghost" size="sm" className="text-white/60 hover:text-white" data-testid="link-back-owner">
      ...
      {/* Hero Header */}
      <Card className="glass-panel bg-gradient-to-br from-orange-500/15 via-rose-500/10 to-violet-500/10 border-orange-500/30 overflow-hidden relative">
        ...
        <CardContent className="p-6 sm:p-8 relative z-10">
          <div className="flex items-start gap-4">
            <div className="size-14 sm:size-16 rounded-2xl bg-gradient-to-br from-orange-500/30 to-rose-500/20 flex items-center justify-center border border-orange-500/20 shrink-0">
              <Palette className="size-7 sm:size-8 text-orange-400" />
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">Brand Assets</h1>
              <p className="text-sm text-white/60 mt-1">Download logos, view brand colors, and access design guidelines for your franchise.</p>
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                <Badge className="bg-orange-500/20 text-orange-300 border-orange-500/30 text-[10px]">
                  <Sparkles className="size-3 mr-1" /> Happy Eats
                </Badge>
                <Badge className="bg-violet-500/20 text-violet-300 border-violet-500/30 text-[10px]">
                  <Shield className="size-3 mr-1" /> Trust Layer
                </Badge>
                <Badge className="bg-white/10 text-white/60 border-white/10 text-[10px]">
                  <FileImage className="size-3 mr-1" /> {BRAND_LOGOS.length} Assets
                </Badge>
              </div>
            </div>
          </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-1.5 mt-3">
                        <Button size="sm" onClick={() => handleDownload(logo.src, `${logo.id}.${logo.formats[0].toLowerCase()}`)}
                          ...
                          className="h-8 text-xs bg-white/10 text-white border border-white/20 hover:bg-white/20 flex-1"
      ...
      <Card className="glass-panel border-l-4 border-l-violet-400">
        ...
        <CardContent>
          <Carousel opts={{ align: "start", loop: true }} className="w-full">
            <CarouselContent className="-ml-3">
              {TYPOGRAPHY.map(font => (
                <CarouselItem key={font.name} className="pl-3 basis-[80%] sm:basis-1/2 lg:basis-[45%]">
                  <div className="rounded-xl bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 border border-violet-500/20 p-5 h-full">
            ...
            <CarouselContent className="-ml-2">
              {GUIDELINES.map((item, i) => (
                <CarouselItem key={i} className="pl-2 basis-[70%] sm:basis-1/2 lg:basis-1/3">
                  <div className="rounded-xl bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 p-4 h-full">
      ...
      {/* Quick Links */}
      <Card className="glass-panel bg-gradient-to-br from-emerald-500/10 via-transparent to-cyan-500/10">
        <CardContent className="p-5">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-xs text-white/50">Quick links:</span>
            <Link href="/marketing-materials">
              <Button size="sm" variant="ghost" className="h-7 text-xs text-white/50 hover:text-white" data-testid="link-business-cards">
                <FileImage className="size-3 mr-1" /> Business Card Designer
              </Button>
            </Link>
            <Link href="/marketing">
              <Button size="sm" variant="ghost" className="h-7 text-xs text-white/50 hover:text-white" data-testid="link-marketing-hub">
                <Sparkles className="size-3 mr-1" /> Marketing Hub
              </Button>
            </Link>
          </div>
        </CardContent>
